...
func (o *jsiiProxy_ObservabilityPipelineConfigDestinationAmazonSecurityLakeOutputReference) validatePutBufferParameters(value interface{}) error {
	...
	case *[]*ObservabilityPipelineConfigDestinationAmazonSecurityLakeBuffer:
		value := value.(*[]*ObservabilityPipelineConfigDestinationAmazonSecurityLakeBuffer)
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
			}
	...
	case []*ObservabilityPipelineConfigDestinationAmazonSecurityLakeBuffer:
		value_ := value.([]*ObservabilityPipelineConfigDestinationAmazonSecurityLakeBuffer)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*ObservabilityPipelineConfigDestinationAmazonSecurityLakeBuffer; received %#v (a %T)", value, value)
		}
	}
